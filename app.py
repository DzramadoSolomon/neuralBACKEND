...
def predict():
    ...
    try:
        ...
        logger.info("=== COMPLETE RESPONSE DEBUG ===")
        logger.info(f"Response structure:")
        logger.info(f"- results count: {len(response_data['results'])}")
        logger.info(f"- total_defects_found: {response_data['summary']['total_defects_found']}")
        for i, result in enumerate(response_data['results']):
            logger.info(f"  Result {i}: image_id={result['image_id']}, predictions_type={type(result.get('predictions'))}, predictions_len={len(result.get('predictions', []))}")
